...
function renderFenced(lang: string, code: string): string {
  ...
  // Chatllm `--syntax-*` palette in webview.css so the same colors as the
...
function loadMermaid(): Promise<{ render: (source: string) => Promise<string> } | null> {
  if (!mermaidLoader) {
    mermaidLoader = new Promise((resolve) => {
      const existing = (window as unknown as { ChatllmMermaid?: { render: (source: string) => Promise<string> } }).ChatllmMermaid;
      ...
      tag.src = src;
      tag.onload = () => {
        const lib = (window as unknown as { ChatllmMermaid?: { render: (source: string) => Promise<string> } }).ChatllmMermaid;
        resolve(lib ?? null);
      };
